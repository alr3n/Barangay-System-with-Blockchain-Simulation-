    ...
    <form method="POST" action="{{ route('households.update', $household) }}" novalidate>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-100 bg-gray-50 flex items-center justify-between">
                ...
                <span class="font-mono text-xs text-gray-500 bg-gray-100 px-2 py-1 rounded">{{ $household->household_code }}</span>
            </div>
            <div class="p-6 space-y-4">
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1.5">Full Address <span class="text-red-500">*</span></label>
                    <input type="text" name="address" value="{{ old('address', $household->address) }}" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" required>
                </div>
                    ...
                    <div>
                        ...
                        <select name="purok" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                            ...
                            @foreach(['Purok 1','Purok 2','Purok 3','Purok 4','Purok 5','Purok 6'] as $p)
                                <option value="{{ $p }}" @selected(old('purok', $household->purok)==$p)>{{ $p }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1.5">Street</label>
                        <input type="text" name="street" value="{{ old('street', $household->street) }}" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                    </div>
                </div>
                <div>
                    ...
                    <select name="house_type" id="house_type" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" required>
                        @foreach(['owned'=>'Owned','rented'=>'Rented','shared'=>'Shared','other'=>'Other'] as $val => $label)
                            <option value="{{ $val }}" @selected(old('house_type', $household->house_type)==$val)>{{ $label }}</option>
                        @endforeach
                    </select>
                ...
                <div id="other_type_field" class="hidden">
                    ...
                    <input type="text" name="house_type_other" value="{{ old('house_type_other', $household->house_type_other) }}"
                ...
                <div id="owner_fields" class="hidden space-y-4 border border-blue-100 bg-blue-50 rounded-xl p-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="owner_name" value="{{ old('owner_name', $household->owner_name) }}"
                    ...
                    <div>
                        ...
                        <input type="text" name="owner_contact" value="{{ old('owner_contact', $household->owner_contact) }}"
                    ...
                    <div>
                        ...
                        <input type="text" name="owner_address" value="{{ old('owner_address', $household->owner_address) }}"
        ...
        <div class="flex items-center gap-3 mt-5">
            <button type="submit" class="bg-primary text-white px-6 py-2.5 rounded-lg text-sm font-medium hover:bg-blue-900">Update</button>
            <a href="{{ route('households.show', $household) }}" class="border border-gray-300 text-gray-600 px-6 py-2.5 rounded-lg text-sm hover:bg-gray-50">Cancel</a>
        </div>
    </form>
</div>
